...
| `HASH_AGG_HIDES_GROUPING` | The `GROUP BY` is done by a `Hash Match (Aggregate)`, which keeps its grouping columns as hash keys and writes **no `<GroupBy>`** into the plan -- so index tooling that reads the plan cannot tell what the query groups by, and suggests no key that would serve it. Common, because a `GROUP BY` with no `ORDER BY` normally hashes. **Not a defect in the query** -- it never changes the verdict to `hold`, and it is deliberately the **last** recommendation printed, because the last of its three suggested steps is a hint. Fix order: build the proposed missing index and re-capture; add the `ORDER BY` if the query needs one; and only then, *to look and not to ship*, compile once with `OPTION (ORDER GROUP)` under `SET SHOWPLAN_XML ON` and pass that plan to `usp_IndexAnalysis @StatementPlanXml`. Take the hint back out -- alone it forces a sort that was not there, and once the index exists it does nothing. Fixtures `f23_hash_agg` (fires), `f24_stream_agg` (same query, streamed -- must not fire), `f25_hash_agg_with_groupby` (a hash aggregate *and* a `<GroupBy>` -- pins the guard). |
